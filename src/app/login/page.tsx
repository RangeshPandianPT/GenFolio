...
import { ChevronLeft, Github, Mail, Lock } from "lucide-react";
...
export default function Login() {
  ...
  // Note: we'd ideally initialize auth outside, but this works for demo
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // We would show a toast error here in a real app
    } finally {
      setIsLoading(false);
    }
  };
        ...
        >
          <div className="glass-panel p-8 rounded-2xl border border-border/50 shadow-2xl space-y-8">
            <div className="text-center space-y-2">
              ...
              <p className="text-sm text-muted-foreground">Sign in to your GenFolio account to continue.</p>
            </div>

            <div className="space-y-4">
              <button 
                onClick={handleGoogleLogin}
                className="w-full flex items-center justify-center gap-3 px-4 py-2.5 rounded-xl border border-border/50 bg-secondary/30 hover:bg-secondary/60 transition-colors font-medium text-sm"
              >
